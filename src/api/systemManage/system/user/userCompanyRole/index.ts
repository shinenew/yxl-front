 ...
 * 用户所在公司角色信息
 */
class UserCompanyRole extends ApiBasic<IOptions, IData> {

    public async api(option: IOptions): Promise<Response<IData>> {
        const req: Request = new Request(CallType.GET, Urls.USER_COMPANY_ROLE, option);
        let data: Response<any> = await this.callCompany(req);
        return data;
    }
}

export default new UserCompanyRole().run;
