 ...
 * 根据部门获取用户
 */
class UpdateDep extends ApiBasic<IOptions, IData> {

    /** 入口 */
    public async api(option: IOptions): Promise<Response<IData>> {

        const req: Request = new Request(CallType.POST, Urls.UPDATE_DEPARTMENT, this.working(option));
...
export default new UpdateDep().run;
