 ...
 * 根据部门获取用户
 */
class DepartmentUser extends ApiBasic<IOptions, IData> {

    /** 入口 */
    public async api(option: IOptions): Promise<Response<IData>> {

        const req: Request = new Request(CallType.GET, Urls.FIND_DEPARTMENT_USER, this.working(option));
...
export default new DepartmentUser().run;
