 ...
 * 根据部门获取用户
 */
class SetUserDep extends ApiBasic<IOptions, IData> {

    /** 入口 */
    public async api(option: IOptions): Promise<Response<IData>> {

        const req: Request = new Request(CallType.POST, Urls.USER_CREATE, this.working(option));
...
export default new SetUserDep().run;
